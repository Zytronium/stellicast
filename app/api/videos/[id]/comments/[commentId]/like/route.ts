...
export async function POST(req: Request, context: RouteContext) {
  try {
    ...
    // Cheap existence check — ensure comment exists and belongs to this video
    ...
    const rpcRes = await admin.rpc('user_comment_like', { p_user: user.id, p_comment: commentId }).single();
    ...
    if (rpcRes.error) {
      ...
      console.error('user_comment_like RPC error:', rpcRes.error);
      return json({ error: 'Internal server error' }, 500);
    }

    const data = rpcRes.data as { liked: boolean; like_count: number; dislike_count: number } | null;
    if (!data) {
      console.error('user_comment_like returned no data:', rpcRes);
      return json({ error: 'Internal server error' }, 500);
    }

    return json({ success: true, liked: data.liked, like_count: data.like_count, dislike_count: data.dislike_count });
  } catch (err: any) {
    const msg = String(err?.message || err).toLowerCase();
    if (msg.includes('rate_limited')) return json({ error: 'Rate limit exceeded' }, 429);
    console.error('Unexpected error in comment like route:', err);
    return json({ error: 'Internal server error' }, 500);
  }
}
